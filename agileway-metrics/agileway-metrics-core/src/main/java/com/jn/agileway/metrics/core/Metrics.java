...
import com.jn.agileway.metrics.core.composite.CompositeMeterRegistry;
...
public class Metrics {
    ...
    /**
     * Register a gauge that reports the size of the {@link Collection}. The
     ...
     * that calling {@link Collection#size()} can be expensive for some
    ...
    /**
     * Register a gauge that reports the size of the {@link Map}. The
     * registration will keep a weak reference to the collection so it will not prevent
     * garbage collection. The collection implementation used should be thread safe. Note
     * that calling {@link Map#size()} can be expensive for some collection
