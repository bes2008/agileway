...
public abstract class MeterRegistry {
    ...
    private final List<BiConsumer<Id, String>> meterRegistrationFailedListeners = new CopyOnWriteArrayList<>();
     ...
     * {@link #newLongTaskTimer(Id, DistributionStatisticConfig)} instead.
     */
     ...
     */
    protected abstract Meter newMeter(Id id, Meter.Type type, Iterable<Measurement> measurements);
     ...
     * @return The meter.
     */
    Meter register(Id id, Meter.Type type, Iterable<Measurement> measurements) {
    ...
    private <M extends Meter> M registerMeterIfNecessary(Class<M> meterClass, Id id,
                                                         Function<Id, M> builder, Function<Id, M> noopBuilder) {
    ...
    private Meter getOrCreateMeter(@Nullable DistributionStatisticConfig config,
                                   ...
                                   Id mappedId, Function<Id, ? extends Meter> noopBuilder) {
    ...
    public class More {
         ...
         * @return A new or existing long task timer.
         */
        LongTaskTimer longTaskTimer(Id id, DistributionStatisticConfig distributionStatisticConfig) {
         ...
         * @return A new or existing time gauge.
         */
        <T> TimeGauge timeGauge(Id id, @Nullable T obj, TimeUnit timeFunctionUnit,
                                ToDoubleFunction<T> timeFunction) {
     ...
     * @since 1.6.0
     */
    protected void meterRegistrationFailed(Id id, @Nullable String reason) {
