 ...
 */
package com.jn.agileway.metrics.core.config.validate;

import com.jn.agileway.metrics.core.config.MeterRegistryConfig;
...
public interface Validated<T> extends Iterable<Validated<T>> {
    ...
    default Validated<T> nonBlank() {
        return invalidateWhen(t -> Strings.isBlank(t.toString()), "cannot be blank", InvalidReason.MISSING);
    }
