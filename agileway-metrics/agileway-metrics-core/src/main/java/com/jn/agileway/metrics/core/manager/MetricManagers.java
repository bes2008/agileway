 ...
 */
public class MetricManagers {

    public static final MetricManager NOP_METRIC_MANAGER = new NoopMetricManager();
    private static final String BINDER_CLASS = "com.jn.agileway.metrics.core.MetricManagerBinder";
    private static volatile MetricManager iMetricManager;
    ...
    public static Meter getMeter(String group, MetricName name) {
        MetricManager manager = getIMetricManager();
    ...
    /**
     * get dynamically bound {@link MetricManager} instance
     *
     * @return the {@link MetricManager} instance bound
     */
    @SuppressWarnings("unchecked")
    public static MetricManager getIMetricManager() {
        if (iMetricManager == null) {
            synchronized (MetricManagers.class) {
                if (iMetricManager == null) {
                    try {
                        Class binderClazz = MetricManagers.class.getClassLoader().loadClass(BINDER_CLASS);
                        ...
                        iMetricManager = (MetricManager) getMetricManager.invoke(binderObject);
                    } catch (Exception e) {
